...
(defn notify-clients [{:keys [com.htm2/chat-clients]} tx]
...
(defn navbar [{:keys [session biff/db] :as ctx}]
  [:nav.navbar {:role "navigation" :aria-label "main navigation" :data-signals "{menu_open: false}"}
   [:div.navbar-brand
    ...
    [:a {:role "button" :class "navbar-burger" :data-class "{'is-active': $menu_open}" :aria-label "menu" :aria-expanded "false" :data-target "navbarBasicExample" :data-on-click "$menu_open = ! $menu_open"}
     [:span {:aria-hidden "true"}]
     [:span {:aria-hidden "true"}]
     [:span {:aria-hidden "true"}]
     [:span {:aria-hidden "true"}]]]
   [:div.navbar-menu {:id "navbarBasicExample" :data-class "{'is-active': $menu_open}"}
    [:div {:class "navbar-start"}
     [:a {:class "navbar-item"} "Documentation"]
     [:a {:class "navbar-item"} "About"]
     ]
    [:div {:class "navbar-end"}
     (if (nil? (:user-email session))
       ...
       {:action "/auth/signout"
        :class "inline"}
       [:a {:class "button is-light"} "Log out"])]]
       )
     ]]]
  )
